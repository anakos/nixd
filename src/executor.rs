...
impl Executor
{
    ...
    pub async fn run_command<F>(&self, label: String, cmd: F)
    -> nixd::Result<()>
    where
    ...
    -> nixd::Result<()>
    {
        ...
        if let Err(e) = self.handle.await {
            Err(nixd::Error::BadShit { message: format!("error writing to stdout: {}", e) }) 
        } else {
            Ok(())
        }
    }
...
impl ProgressIndicator
{
    fn start(my_sender: sync::Sender<Status>) -> ProgressIndicator
    {
        ...
        let mut my_stream  = _kill_switch.stop_token().stop_stream(stream::interval(std::time::Duration::from_millis(500)).fuse());
        ...
        ProgressIndicator { _handle, _kill_switch, }
    }
}
