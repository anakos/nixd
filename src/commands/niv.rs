...
pub fn add_packages(packages: &[String]) -> nixd::Result<()> {
    for package in packages {
        super::exec("niv", |mut cmd| cmd.args(&["add", package]).output())?;
    }
...
pub fn update(branch: nixd::NixBranch) -> nixd::Result<()> {
    super::exec("niv", |mut cmd| {
        cmd.args(&["update", "nixpkgs", "-b", &branch.to_string()])
            .output()
    })?;
...
mod tests {
    #[test]
    fn check_requires_niv() -> crate::types::Result<()> {
        super::check()?;
        Ok(())
    }
}
